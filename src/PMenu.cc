...
PMenu::buildMenuCalculate(void)
{
    ...
    Theme::PMenuData *data = _theme->getMenuData(_name);
    _item_width_max += data->getPad(PAD_LEFT) + data->getPad(PAD_RIGHT);
    ...
    if (_has_submenu) {
        _item_width_max += data->getPad(PAD_RIGHT)
            + data->getTextureArrow(OBJECT_STATE_FOCUSED)->getWidth();
    }
    ...
    _item_height = std::max(data->getFont(OBJECT_STATE_FOCUSED)->getHeight(), _icon_height)
      + data->getPad(PAD_UP) + data->getPad(PAD_DOWN);
    _separator_height = data->getTextureSeparator(OBJECT_STATE_FOCUSED)->getHeight();
...
PMenu::buildMenuCalculateMaxWidth(unsigned int &width, unsigned int &height)
{
    ...
    for (; it != _items.end(); ++it) {
        ...
        width = _theme->getMenuData(_name)->getFont(OBJECT_STATE_FOCUSED)->getWidth((*it)->getName().c_str());
...
PMenu::buildMenuRenderState(Pixmap &pix, ObjectState state)
{
    ...
    PTexture *tex = data->getTextureMenu(state);
    tex->render(pix, 0, 0, getChildWidth(), getChildHeight());

    PFont *font = data->getFont(state);
    font->setColor(data->getColor(state));
...
{
    PTexture *tex;
    Theme::PMenuData *md = _theme->getMenuData(_name);
